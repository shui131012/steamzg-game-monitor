...
发现新的游戏文章后，会通过 Telegram 发送通知，通知里包含游戏标题和链接。已经推送过的文章会保存在 `data/seen_games.json`，避免重复推送。
...
| 文件 | 作用 |
| --- | --- |
| `src/monitor.py` | 主程序：抓取网页、检测新文章、发送 Telegram、更新历史记录 |
...
不会。默认最多保留 500 条历史记录，可以在 `.github/workflows/monitor.yml` 里修改：
